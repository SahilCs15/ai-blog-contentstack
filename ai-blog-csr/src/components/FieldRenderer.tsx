// Universal field renderer. Given an entry, it walks every field (via
// inferFields) and renders a labelled row per field — field name, inferred
// type badge, and a kind-appropriate value. New fields appear automatically.
//
// Each value carries its data-cslp tag so Live Preview highlights/edits work
// the same here as on the hand-built pages.
...
/** Turn a nested plain object (group / block / global field) into render rows. */
...
/**
 * Render one entry: title heading + every field through the dynamic grid.
 */
...
/**
 * Render every field of every entry. Each entry is its own card; new fields and
 * new entries both appear automatically (schema-less, driven by the data shape).
 */
