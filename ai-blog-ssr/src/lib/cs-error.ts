...
/**
 * SSR helper: turn a CsErrorDetail into a real Error whose message is the
 * serialized detail. Next.js preserves Error.message across the server →
 * error-boundary boundary, so error.tsx can parse it back with parseCsError().
 */
const CS_PREFIX = 'CSERR::'

export function asThrowable(detail: CsErrorDetail): Error {
  return new Error(CS_PREFIX + safeStringifyExport(detail))
}

export function parseCsError(err: unknown): CsErrorDetail {
  const msg = err instanceof Error ? err.message : typeof err === 'string' ? err : ''
  if (msg.startsWith(CS_PREFIX)) {
    try {
      return JSON.parse(msg.slice(CS_PREFIX.length)) as CsErrorDetail
    } catch {
      /* fall through */
    }
  }
  return toCsError(err)
}

function safeStringifyExport(v: unknown): string {
  try {
    return JSON.stringify(v)
  } catch {
    return JSON.stringify({ message: String(v) })
  }
}
