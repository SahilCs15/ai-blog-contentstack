...
const PROMISE_ICONS = [<TruckIcon key="t" />, <ReturnIcon key="r" />, <ShieldIcon key="s" />, <MedalIcon key="m" />]
  ...
  return (
    <section className={`ss-feature${right ? ' ss-feature--right' : ''}`} {...sectionTag}>
      <div className="ss-feature__media">
        {b.image?.url ? (
          // eslint-disable-next-line @next/next/no-img-element
          <img src={imageUrl(b.image.url, 900)} alt={b.heading || ''} loading={imgLoading()} {...edit(b.image.$, 'url')} />
        ) : null}
      </div>
      ...
      >
        {imgs.map((im, i) => (
          <figure className="ss-lookbook__cell" key={i} {...edit(b.$, `images__${i}`)}>
            {im?.url ? (
              // eslint-disable-next-line @next/next/no-img-element
              <img src={imageUrl(im.url, 700)} alt={b.caption || ''} loading={imgLoading()} {...edit(im.$, 'url')} />
            ) : null}
          </figure>
        ))}
      </div>
...
function heroHeading(t?: string, hl?: string, tag?: EditTag) {
  const text = t ?? ''
  if (!hl || !text.includes(hl)) return text
  const [b, a] = text.split(hl)
  return (<>{b}<span className="ss-accent" {...(tag ?? {})}>{hl}</span>{a}</>)
}

export default async function ShoeStorePage({ params, searchParams }: Props) {
  ...
  const page = await getShoeLanding(live_preview, locale)

  if (!page) {
    return (
      <section className="section">
        <div className="section__head"><h2>Shoe store not found</h2><p className="section__sub">Publish a shoe_landing entry to this environment.</p></div>
      </section>
    )
  }
  ...
  return (
    <div className="shoestore">
      {(page.free_shipping || page.drop_date || page.collection) && (
      ...
      <header className="ss-nav">
        <div className="ss-nav__inner">
          <a className="ss-nav__brand" href="#top"><span className="ss-nav__mark"><BagIcon /></span><span {...edit(page.$, 'brand_name')}>{slot(brand)}</span></a>
      ...
      <section className="ss-hero" id="about">
        <div className="ss-hero__text">
          ...
          <h1 className="ss-hero__heading" {...edit(page.$, 'hero_heading')}>{heroHeading(page.hero_heading, page.hero_highlight, edit(page.$, 'hero_highlight'))}</h1>
      ...
      <section className="ss-products" id="products">
        <div className="ss-section-head">
          <div><span className="ss-eyebrow">Bestsellers</span><h2>Popular Products</h2></div>
      ...
      {page.testimonials?.length ? (
        <section className="ss-testimonials" id="testimonials">
          <div className="ss-section-head">
            <div><span className="ss-eyebrow">Reviews</span><h2>What Our Customers Say</h2></div>
          </div>
            ...
            {page.testimonials.map((t, i) => (
              <figure className="ss-quote" key={i} {...edit(page.$, `testimonials__${i}`)}>
                ...
                <figcaption className="ss-quote__by">
                  {t.avatar?.url ? (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img src={imageUrl(t.avatar.url, 96)} alt={t.customer_name} {...edit(t.avatar.$, 'url')} />
                  ) : <span className="ss-quote__avatar" aria-hidden="true" />}
      ...
      <footer className="ss-footer">
        ...
        <div className="ss-footer__bar">
          ...
          <div className="ss-footer__legal"><a href="#">Privacy</a><a href="#">Terms</a><a href="#">Cookies</a></div>
        </div>
      </footer>
    </div>
  )
