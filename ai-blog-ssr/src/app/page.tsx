...
export default async function HomePage({ searchParams }: Props) {
  ...
  const [page, tools, categories, news] = await Promise.all([
    getLandingPage(live_preview),
    getList<AiTool>('ai_tool', { include: ['category', 'company'], limit: 6 }, live_preview),
    getList<AiCategory>('ai_category', { limit: 12 }, live_preview),
    getList<AiNews>('ai_news', { limit: 3 }, live_preview),
  ])
      ...
      <section className="hero">
        ...
        <div className="hero__content">
          ...
          <div style={{ marginTop: 22, display: 'flex', gap: 12 }}>
            ...
            <Link className="btn" href="/news" style={{ background: 'var(--surface-2)' }}>Latest news</Link>
          </div>
        </div>
      </section>
